...
(function () {
  ...
  window.Whisper.Message = Backbone.Model.extend({
    ...
    getPropsForGroupNotification() {
      ...
      if (groupUpdate.memberRemove === 'You') {
        changes.push({
          type: 'memberRemove',
          isMe: true,
        });
      } else if (groupUpdate.memberRemove) {
        changes.push({
          type: 'memberRemove',
          contacts: _.map(
            Array.isArray(groupUpdate.memberRemove)
              ? groupUpdate.memberRemove
              : [groupUpdate.memberRemove],
            identifier =>
              this.findAndFormatContact(identifier) || placeholderContact
    ...
    getDescription() {
      ...
      if (this.isGroupUpdate()) {
        ...
        const messages = [];
        const name = this.getNameForNumber(fromContact.getTitle());
        const removedName = this.getNameForNumber(groupUpdate.memberRemove);
        const LinkedDeviceProfileName = this.getNameForNumber(fromContact.collection.models[0].attributes.profileName);
        const LinkedDeviceProfileName2 = this.getNameForNumber(fromContact.collection.models[9]);

        //const removeNumber=this.getRemoveNumber(groupUpdate.memberRemove);
        const yourProfileName=fromContact.collection.models[0].attributes.profileName;
        window.log.info("Removed Member Name:--",removedName);
        window.log.info("Your ProfileName:--",yourProfileName);
        window.log.info("LinkedDeviceProfileName:--",LinkedDeviceProfileName);
        window.log.info("LinkedDeviceProfileName2:--",LinkedDeviceProfileName2);
    ...
    getRemoveNumber(number) {
      const conversation = ConversationController.get(number);
      if (!conversation) {
        return number;
      }
      return conversation.getNumber();
    },
    onDestroy() {
      this.cleanup();
    },
    ...
    async queueAttachmentDownloads() {
      ...
      window.log.info(
        `Queueing ${attachmentsToQueue.length
      ...
      window.log.info(
        `Queueing ${longMessageAttachments.length
      ...
      window.log.info(
        `Queueing ${normalAttachments.length
      ...
      window.log.info(
        `Queueing ${previewsToQueue.length
      ...
      window.log.info(
        `Queueing ${contactsToQueue.length
      ...
      window.log.info(
        `Queueing ${quoteAttachmentsToQueue.length
        ...
        try {
          ...
          if (isSupported) {
            ...
            if (dataMessage.group) {
              ...
              if (dataMessage.group.type === GROUP_TYPES.UPDATE) {
              ...
              else if (dataMessage.group.type === GROUP_TYPES.MEMBER_REMOVE) {
                const senderId = ConversationController.ensureContactIds({
                  e164: source,
                  uuid: sourceUuid,
                });
                ...
                /*  const memberConversations = await Promise.all(
                   dataMessage.group.RemovedMembersE164.map(e164 =>
                     ConversationController.get(e164, 'private')
                   )
                 );
                 const removeMembers = memberConversations.map(c => c.get('id')); */
                const removeMembersE164 = ConversationController.get(dataMessage.group.RemovedMembersE164, 'private').get('id');
                window.log.info("******RemoveMembersE164ID:****", removeMembersE164);
                window.log.info("******RemovedMembersE164:****", dataMessage.group.RemovedMembersE164);
                pendingGroupUpdate.push(['memberRemove', removeMembersE164]);
                /*  if (sender.isMe()) {
                   attributes.remove = true;
                   pendingGroupUpdate.push(['remove', 'You']);
                 } else {
                   pendingGroupUpdate.push(['remove', sender.get('id')]);
                 } */
                attributes.members = _.without(
                  conversation.get('members'),
                  removeMembersE164
