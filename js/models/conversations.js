 ...
 async getProfile(id) {
   window.log.info("all keys checked inside inside",id)
    ...
    async setProfileAvatar(avatarPath) {
      window.log.info('setProfileAvatar avatarPath',avatarPath);
      if (!avatarPath) {
        return;
      }

      const avatar = await textsecure.messaging.getAvatar(avatarPath);
      window.log.info('setProfileAvatar avatar',avatar);
    ...
    async setProfileKey(profileKey) {
      window.log.info('setProfileAvatar profileKey',profileKey);
    ...
    async dropProfileKey() {
      if (this.get('profileKey')) {
        ...
        if (profileAvatar && profileAvatar.path) {
          window.log.info('indside if profileAvatar',profileAvatar);
          window.log.info('indside if profileAvatar.path ',profileAvatar.path);
    ...
    getAvatarPath() {
      const avatar = this.isMe()
        ...
        window.log.info('indside getAvatarPath avatar.path',avatar);
