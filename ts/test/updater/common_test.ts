...
`;
  ...
  describe('#getUpdateFileName', () => {
    it('successfully gets version', () => {
      assert.strictEqual(
        ...
        'dosto-desktop-win-1.23.2.exe'
      );
      assert.strictEqual(
        getUpdateFileName(mac),
        'dosto-desktop-mac-1.23.2.zip'
      );
      assert.strictEqual(
        getUpdateFileName(windowsBeta),
        'dosto-desktop-beta-win-1.23.2-beta.1.exe'
      );
      assert.strictEqual(
        getUpdateFileName(macBeta),
        'dosto-desktop-beta-mac-1.23.2-beta.1.zip'
      );
    });
  });
  ...
  describe('#isUpdateFileNameValid', () => {
    it('returns true for normal filenames', () => {
      assert.strictEqual(
        isUpdateFileNameValid('dosto-desktop-win-1.23.2.exe'),
        true
      );
      assert.strictEqual(
        isUpdateFileNameValid('dosto-desktop-mac-1.23.2-beta.1.zip'),
        true
      );
    });
    it('returns false for problematic names', () => {
      assert.strictEqual(
        isUpdateFileNameValid('../dossto-desktop-win-1.23.2.exe'),
        false
      );
      assert.strictEqual(
        isUpdateFileNameValid('%sdosto-desktop-mac-1.23.2-beta.1.zip'),
        false
      );
      assert.strictEqual(
        isUpdateFileNameValid('@dosto-desktop-mac-1.23.2-beta.1.zip'),
