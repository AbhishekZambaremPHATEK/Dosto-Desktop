...
export default class MessageSender {
  ...
  async getProfile(
    ...
    if (accessKey) {
     // const unauthOptions = {    ...options,    accessKey,    };
      return this.server.getProfileUnauth(number,{ accessKey });
    }
    window.log.info('server getProfile:-',this.server.getProfile(number))
...
/* async getProfile(number, { accessKey } = {}) {
  ...
  async getAvatar(path: string) {
    window.log.info('getAvatar inside sendMessage:-',path)
  ...
  async sendRequestContactSyncMessage(options?: SendOptionsType) {
    ...
    if (myDevice !== 1 && myDevice !== '1') {
      const request = new window.textsecure.protobuf.SyncMessage.Request();
      request.type =window.textsecure.protobuf.SyncMessage.Request.Type.CONTACTS;
      const syncMessage = this.createSyncMessage();
      syncMessage.request = request;
      const contentMessage = new window.textsecure.protobuf.Content();
      contentMessage.syncMessage = syncMessage;
  ...
  //  proto.group.group_type = group_type;
  ...
  async setGroupAvatar(
    ...
    proto.group.type = window.textsecure.protobuf.GroupContext.Type.UPDATE;
    proto.group.membersE164 = groupIdentifiers;
    window.log.info('proto inside setGroupAvatar:-',proto);
    window.log.info('group inside setGroupAvatar:-',proto.group );
    window.log.info('groupId inside setGroupAvatar:-',proto.group.id);
    window.log.info('proto.group.type inside setGroupAvatar:-',proto.group.type);
    window.log.info('proto.group.membersE164 inside setGroupAvatar:-',proto.group.membersE164);
